...
def get_torrent_ids(page_url: str, headers: dict) -> Set[str]:
    ...
    try:
        ...
        if not response.text:
            log.warning(f"第 {page_num} 页返回空内容")
            return set()

        torrent_ids = set(re.findall(r'href="details\.php\?id=(\d+)&amp;hit=1', response.text))
        log.info(f"第 {page_num} 页解析到 {len(torrent_ids)} 个种子")
        return torrent_ids

    except requests.exceptions.RequestException as e:
        log.error(f"获取第 {page_num} 页失败: {str(e)}")
        return set()
...
def say_thanks(torrent_id: str, count: int, thanks_url: str, headers: dict) -> bool:
    ...
    try:
        ...
        response.raise_for_status()
        log.info(f"种子 #{count}; ID: {torrent_id} - 感谢表示成功")
        return True

    except requests.exceptions.RequestException as e:
        log.error(f"为种子ID {torrent_id} 说谢谢失败: {str(e)}")
        return False
...
def thank_batch(all_torrent_ids: Set[str], headers: dict, thanks_url: str):
    ...
    for count, torrent_id in enumerate(all_torrent_ids, 1):
        ...
        if not success:
            ...
            if REQUEST_DELAY > 0:
                log.info(f"等待 {REQUEST_DELAY} 秒后继续...")
                time.sleep(REQUEST_DELAY)
    # 打印失败的种子 ID 集合
    if failed_torrent_ids:
        log.warning(f"以下种子感谢失败: {failed_torrent_ids}")
        log.info("提示：可以复制这些 ID 并遍历调用 say_thanks() 函数重试，"
...
def process_all_pages():
    ...
    if not PT_COOKIE:
        log.error("请在 config.env 中配置有效的 Cookie")
        return
    ...
    log.info(f"开始处理 {PT_DOMAIN} 的页面，第 {PAGE_START} 到 {PAGE_END} 页")

    # 收集所有种子ID
    for page_url in page_urls:
        log.debug(f"正在处理页面: {page_url}")
        torrent_ids = get_torrent_ids(page_url, headers)
        all_torrent_ids.update(torrent_ids)

    if not all_torrent_ids:
        log.warning("没有找到任何种子ID，请检查配置和页面内容")
        return

    log.info(f"共找到 {len(all_torrent_ids)} 个种子，开始说谢谢...")
